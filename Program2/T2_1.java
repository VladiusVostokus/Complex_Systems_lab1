...
public class T2_1 extends Thread {
    private int h;
    private Ops2 O;
    private int id;

    public T2_1(Ops2 o1, int n, int p, int threadId) {
        h = n / p;
        O = o1;
        id = threadId;
    }
    ...
    public void run() {
        ...
        O.sortVec(O.data.X);
        try {
            O.data.sem2_1.acquire();;
        } catch(InterruptedException e) {
            e.printStackTrace();
        }
        O.mergeVec(O.data.X1, 0, O.data.H, O.data.H * 2);
        try {
            O.data.sem3_1.acquire();
        } catch(InterruptedException e) {
            e.printStackTrace();
        }
        O.mergeVec(O.data.X1, 0, O.data.H * 2, O.data.H * 4);
        try {
